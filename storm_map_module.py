...
def generate_contour_ice(mapobj, data_ice_metric, centroid_coordinate, nb_class = 2, contour_name = None, df_flood = None):
    ...
    #cm     = branca.colormap.LinearColormap(colors, vmin=0, vmax=1).to_step(levels)
    #cm     = branca.colormap.LinearColormap(['b','c','g','y','r'], vmin=0, vmax=1)
    cm     = branca.colormap.LinearColormap(['#0000FF', '#FFFF00','#FF0000'], 
                                            vmin=0, 
                                            vmax=0.5, 
                                            caption = contour_name)  
    ...
    # Create a dataframe with fake data
    df = pd.DataFrame({
        'longitude':   np.random.normal(centroid_coordinate[1],    0.075,     10000),
        'latitude':    np.random.normal(centroid_coordinate[0],    0.075,     10000),
        'probability': [random.randint(1,10) for i in range(10000)]})
        #'probability': np.random.normal(0, 1, 10000)})
    ...
    x_arr_len=80
    y_arr_len=72
    x_arr          = np.linspace(-88.5231, -88.3651, x_arr_len)
    y_arr          = np.flipud(np.linspace(41.6859, 41.8039, y_arr_len))
    x_mesh, y_mesh = np.meshgrid(x_arr, y_arr)

    #xl = pd.ExcelFile("./data/Winter_Storm_Ice_weather_metric.xlsx")
    xl = pd.ExcelFile(data_ice_metric)
    
    df_flood = xl.parse("Sheet1")

    z_mesh = np.zeros((y_arr_len, x_arr_len))

    for k1 in range(0,y_arr_len):
        for k2 in range(0,x_arr_len):
            z_mesh[k1,k2]=df_flood.iloc[k1+k2*y_arr_len,0]
...
def generate_contour_wind(mapobj, data_wind_metric, centroid_coordinate, nb_class = 10, contour_name = None,  df_flood = None):
    ...
    #cm     = branca.colormap.LinearColormap(colors, vmin=0, vmax=1).to_step(levels)
    #cm     = branca.colormap.LinearColormap(['b','c','g','y','r'], vmin=0, vmax=1)
    cm     = branca.colormap.LinearColormap(['#0000FF', '#6495ED', '#00FFFF', '#008000', '#8FBC8F', '#FFFF00', '#FFA500', '#FFA07A', '#FF6347',
                                             '#FF0000'], 
                                            vmin=70, 
                                            vmax=80, 
                                            caption = contour_name)  
    ...
    xl = pd.ExcelFile(data_wind_metric)
...
def plot_grid(data_pickle, data_ice_metric, data_wind_metric, default_coord, df_node, df_line, df_flood, df_flood_substation_pv_node):
    ...
    folium_map = generate_contour_ice(folium_map, 
                                 data_ice_metric,
                                 centroid_coord, 
                                 contour_name = 'Ice Thickness',
                                 df_flood = df_flood)
    
    folium_map = generate_contour_wind(folium_map, 
                                 data_wind_metric,
                                 centroid_coord, 
                                 contour_name = 'Wind Speed',
                                 df_flood = df_flood)
    ...
    folium_map = add_point_clusters(folium_map, 
                                   ...
                                   attr = ['Node', 'Ice Thickness','Wind Speed', 'Failure Probability', 'Damage Status', 'lat', 'lon'],  
    ...
    folium_map = add_heatmap(folium_map, 
                 df_line, 
                 attr=['lat', 'lon', 'Start Node', 'End Node', 'Length', 'Ice Thickness','Wind Speed', 'Failure Probability', 'Damage Status'], 
                 radius=20, 
                 heat_name=caption , 
                 info_name='Details of Electric Component' , 
                 colors=colors, 
    ...
    filepath = os.getcwd() + '/output/' + name + '_storm'+ '.html'    
...
#'./data/weather/STORM/IEEE123/Winter_Storm_damge_123.xlsx'
#'./data/weather/STORM/IEEE123/Winter_Storm_Ice_weather_metric.xlsx'
#'./data/weather/STORM/IEEE123/Winter_Storm_Wind_weather_metric.xlsx'


#'./data/weather/STORM/IEEE8500/Winter_Storm_damge_128500.xlsx'
#'./data/weather/STORM/IEEE8500/Winter_Storm_Ice_weather_metric.xlsx'
#'./data/weather/STORM/IEEE8500/Winter_Storm_Wind_weather_metric.xlsx'


def gen_storm_map(data_system_pickle, data_ice_metric, data_wind_metric, data_weather_damage):
    ...
    if '123' in data_system_pickle:
        ...
        df_node_offset['lat'] = df_node_offset['lat'].apply(lambda i: i*42/364647.0 +41.7470)
    ...
    plot_grid(data_system_pickle, data_ice_metric, data_wind_metric, default_coord, df_node, df_flood_line, df_flood, df_flood_substation_pv_node)
